...
class MusicController extends Controller
{
    ...
    public function AddSong(Request $request)
    {
        ...
        if(!isset($id)){
            Session::flash("error", "Please enter the Spotify url of your track.");
        } else if(!isset($request->main_genre)){
            Session::flash("error", "Please select a main genre.");
        } else if(!isset($request->sub_genre)){
            Session::flash("error", "Please select at least 1 sub genre.");
        } else{
            $fee = Prices::where('name', 'promotion')->first();
            $Credit = Credits::where('userId', $id)->first();
            $rest = $Credit->credits - $fee->price*(int)$request->term;
            if($rest < 0){
                Session::flash("error", "Insufficient credit balance.");
            } else{
                ...
                if ($request->hasFile('upfile') && $request->file('upfile')->isValid()){
                    ...
                    $Credit->credits = $rest;
                    $Credit->save();
                    Session::flash('success', 'Your campaign has started. View the status via the campaigns page.');
                } else{
                    Session::flash("error", "Please upload an audio file of your track.");
                }
            }
        }
        return back();
