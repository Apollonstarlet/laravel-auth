...
class SpotifyController extends Controller
{
    ...
    public function ToReview(Request $request)
    {
        ...
        $data['playlist'] = Playlists::where('userId', $userId)->where('status', 'accept')->get();
    ...
    public function Dashbord(Request $request)
    {
        ...
        $data = array();
        $user = $request->user();
        $data['user'] = User::where('email', $user->email)->first();
        $userId = $data['user']->id;
        $data['review_list'] = Reviews::where('userId', $userId)->get();
        $data['like'] = Like::where('userId', $userId)->where('status', 'y')->get();
    ...
    public function AddPlaylist(Request $request){
        ...
        if(!isset($request->url)){
            Session::flash("error", "Failed, please insert Spotify playlist url.");
        } else if(!isset($request->main_genre)){
            Session::flash("error", "Failed, please choose main genre.");
        } else if(!isset($request->sub_genre)){
            Session::flash("error", "Failed, please choose sub genre(s).");
        } else{
           $url = str_replace("https://open.spotify.com/playlist/","", $request->url);
            ...
            if(count($data['images']) > 0){
                if($user->spotifyId == $data['owner']['id']){
